...
static void update_osd(OSDData *osd, int val) {
    ...
    const char *color = "#eed49f"; // Mocha Yellow
...
int main(int argc, char *argv[]) {
    ...
    gtk_css_provider_load_from_data(provider,
        "window { background-color: #24273a; border-radius: 24px; border: 2px solid #eed49f; } "
        "trough { background-color: #363a4f; border-radius: 16px; min-height: 32px; } "
        "progress { background-color: #eed49f; border-radius: 16px; min-height: 32px; transition: all 0.02s ease-out; } ", -1, NULL);
