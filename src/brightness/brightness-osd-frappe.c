...
static void update_osd(OSDData *osd, int val) {
    ...
    const char *color = "#e5c890"; // Frappe Yellow
...
int main(int argc, char *argv[]) {
    ...
    gtk_css_provider_load_from_data(provider,
        "window { background-color: #303446; border-radius: 24px; border: 2px solid #e5c890; } "
        "trough { background-color: #414559; border-radius: 16px; min-height: 32px; } "
        "progress { background-color: #e5c890; border-radius: 16px; min-height: 32px; transition: all 0.05s cubic-bezier(0.16, 1, 0.3, 1); } ", -1, NULL);
