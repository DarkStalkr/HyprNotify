...
static void update_osd(OSDData *osd, int val) {
    ...
    const char *color = "#f9e2af"; // Mocha Yellow
...
int main(int argc, char *argv[]) {
    ...
    gtk_css_provider_load_from_data(provider,
        "window { background-color: #1e1e2e; border-radius: 24px; border: 2px solid #f9e2af; } "
        "trough { background-color: #313244; border-radius: 16px; min-height: 32px; } "
        "progress { background-color: #f9e2af; border-radius: 16px; min-height: 32px; transition: all 0.02s ease-out; } ", -1, NULL);
