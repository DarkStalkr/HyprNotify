...
int main(int argc, char *argv[]) {
    ...
    // Open in RDWR to prevent infinite EOF loop when writers close
    ...
    gtk_css_provider_load_from_data(provider,
        "window { background-color: rgba(24, 24, 37, 0.95); border-radius: 20px; border: 2px solid #f9e2af; box-shadow: 0 4px 15px rgba(0,0,0,0.5); } "
        ...
        "progress { background-image: linear-gradient(to right, #f9e2af 12px, transparent 12px); background-size: 16px 100%; background-color: transparent; border-radius: 4px; min-height: 24px; transition: all 0.1s ease-out; } ", -1, NULL);
