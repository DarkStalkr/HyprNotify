...
static void update_osd(OSDData *osd, int val) {
    ...
    const char *color = "#e0af68"; // Tokyo Night Yellow
...
static gboolean on_fifo_data(GIOChannel *source, GIOCondition condition, gpointer data) {
    ...
    while ((status = g_io_channel_read_line(source, &str, &len, NULL, NULL)) == G_IO_STATUS_NORMAL) {
        if (str) { last_val = atoi(str); g_free(str); }
    }
...
int main(int argc, char *argv[]) {
    ...
    gtk_css_provider_load_from_data(provider,
        "window { background-color: #1a1b26; border-radius: 24px; border: 2px solid #e0af68; } "
        "trough { background-color: #414868; border-radius: 16px; min-height: 32px; } "
        "progress { background-color: #e0af68; border-radius: 16px; min-height: 32px; transition: all 0.05s cubic-bezier(0.16, 1, 0.3, 1); } ", -1, NULL);
